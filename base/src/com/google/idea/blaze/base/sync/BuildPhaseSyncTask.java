...
final class BuildPhaseSyncTask {
  ...
  private void doRun(BlazeContext context) throws SyncFailedException, SyncCanceledException {
    ...
    if (!syncBuildInvoker.supportsHomeBlazerc() && BlazercMigrator.needMigration(project)) {
      context.output(
          SummaryOutput.output(Prefix.INFO, "No .blazerc found at workspace root!").log().dedupe());
      ApplicationManager.getApplication().invokeLater(() -> migrateBlazerc(context));
    }
  ...
  private void migrateBlazerc(BlazeContext context) {
    if (!BlazercMigrator.promptMigration(project)) {
      return;
    }

    if (BlazercMigrator.copyBlazercToWorkspace(project)) {
      context.output(
